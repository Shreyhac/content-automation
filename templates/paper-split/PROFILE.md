...
**One creator, several labels.** The repo history refers to this slot by more than one name; they
all mean the same channel. Do not confuse this creator with the card-reel creator, who is a
different person.
...
| Current grammar | vid47 v2: the paper split band. Reference build: `reference-builds/paper-split-vid47/` |

This creator has the longest and most self-contradicting feedback history of the three. **Read
`HISTORY.md` before applying anything**, and treat only the rules below as standing.

---

## Hard rules

### 1. The creator's footage ships exactly as delivered

"Whatever there is in the video, mirrored or without mirrored, keep that the same."

No hflip even when jacket or shirt text reads backwards. No denoise, no loudnorm, no regrade. The
creator delivers a finished mix and grade. **Transcode only** (crop and scale), `-c:a copy`.

### 2. The creator's face is never cropped

Full head in frame on full-bleed **and** in the split. A cropped crown reads to them as a bug every
time. Make the face read big by **cropping the dead headroom at the ffmpeg stage**, not by zooming
in the composition.

### 3. The hook is an acted micro-story with the creator visible, never a type lockup
...
The creator's self-mixed VO sits around -21 to -25 dB mean, so the house 0.16 to 0.34 range
overpowers it. When they supply a pack (`library/sfx/saas/`), **they expect to hear it**: 80% of
triggers on one film.
...
The creator asked for the final "in the original size I gave it to you" **after** the byte-match
rule (#8) was already satisfied: they meant the 2160×3840 of their own master, not just its file
size. `--resolution=portrait-4k` renders the composition at that size without touching it (Chrome
renders at DPR2; match the aspect, keep the scale an integer multiple). Rebuild the A-roll
assets at 4K first, or a 4K render off 1080p transcodes ships graphics sharp and the face soft.
...
`scale 1` with a paper panel laid over the top, and reposition the subject with a **translate**,
not a crop-and-scale: solve the translate from the crown minimum against the panel edge, and the
chin maximum against y1600. A translate changes their position on screen, not their size.
...
This creator's zoom history reads as contradictory because it is take-dependent, not
preference-dependent. The resolution:
...
- They ask for subtle zoom **animations** (breathing push and pull), not a static crop. Current
...
**Their corrections are round-scoped, not permanent law.** A locked-off frame and a ban on wipes
