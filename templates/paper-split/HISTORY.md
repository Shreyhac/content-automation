# paper-split: shipped work and what each round changed

Newest first. **The most recent approved grammar supersedes older entries.** The creator has
reversed themselves several times; entries are dated so you can tell law from a round-scoped
correction.
...
`paper-split-take2.mp4`, same ten-tool script, second location. Looks like a file swap; budget a
full production round for it, not a re-render.
...
(Vision face-width .242 vs .239) but the subject sat ~490 master-px higher and wandered
vertically nearly twice as far. Reproducing take one's head size put the chin past y1600 on
**11%** of frames against the approved cut's 2%. Widening the crop to 0.95× (`crop=2036:3620:0:0`,
`top:596`, clip inset `484`) gave head 397px and 2.7% over. Search `(top, scale)` against a
percentile constraint; a median-only fit hides the tail.
...
instead: 0→0.55 is the creator at the monitor with no type, and the turn becomes a hard cut into
the split. Composed cover, graphics-free hook, a real event on the turn.
...
**A card safe at the top is not safe at the bottom.** Owner note: the CTA card cut across the
...
was already done: the 2160×3840 of the master. `--resolution=portrait-4k` does this without
...
- **Rebuild the A-roll assets at 4K first, or you ship an upscale.** The project's transcodes
  were 1080×1920; a 4K render off them would leave graphics/type sharp and the face soft.
...
27.87s / 836 frames, 1080×1920. A reference reel existed but the A-roll was a fresh single take.
Owner's only direction: *"start from when he closes the lid of the laptop, that is the hook."*
Scaffolded on hf47 v2's approved paper split. `out/vid50-final.mp4`. Three review rounds, all
against the hook.
...
`s=0.7166`. The subject sits high enough that no crop puts the crown at hf47's y1130 (zoom raises
the head, and zoom-out isn't available past 1:1). Ceiling was y956; built at y900, s=0.690. Check
the inequality before laying out the graphics zone, not after.
...
Built on a three-band grammar with a GitHub-dark palette. The note, with a reference frame from
hf41:
...
Ported hf41's Director's Desk wholesale because the creator had already approved it. Band top
y1080, face presence 45% to 91%. Ten tools as one accordion for 24.5s.
...
the scrim can be deleted entirely and the presenter reads at full brightness.

Also: an abstract 3D glyph reads as a mistake, a 3D glyph carrying a real subject reads as an
idea. Putting the creator's actual face on the U-turn track made the same geometry instantly
legible.
...
First paper-world build for this creator. Source of the Director's Desk system that vid47 v2
ports.
...
- **A masthead BAR is the reliable way to put type behind a person on light footage.** Green type
  over the white wall was invisible and a scrim could not fix it, because the same line also
...
- **But text-behind-person needs a gap wider than the head at that y.** The bar at y528 put the
  word across the crown and it was half-eaten by hair. On a seated take with headroom, move the
...
- **vid10 (07-10).** The creator's footage ships exactly as delivered, mirrored or not. Crop the
  dead headroom at the ffmpeg stage so the face reads big without zooming.
