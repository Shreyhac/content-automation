# fast-cut-ad: shipped work and what each round changed
...
> **Open before you start.** The six-tools film's **v20 was delivered 2026-08-14** to the local
> Reel Review link and the owner has not yet given a verdict. They marked that round *"last
> attempt on this"*. Open `review/data/ad2/comments.json` in the production repo and check for a
> new round before assuming v20 needs more work. All 34 notes recorded there are `resolved` with
> replies written; none are `source: "client"`.

---

## The six-tools film, 9:16 Meta ad, 2160x3840, 37.77s (`hfad2/`, 2026-08-13 to 2026-08-14)

Twenty versions and ten review rounds in about twenty hours. Source: a 34.03s manual cut of a
male A-roll supplied by the owner, plus six client b-roll clips of the same desk, plus the
client's own UI component exports. Reference supplied: four Wispr Flow Meta ads (grammar only, not
tempo), and later a Viktor AI-employee ad (motion language). Plan in `ad2-shot-plan.md`; the
screen recording asked for in `ad2-recording-brief.md` was never made and was not needed, because
the client's own exports covered every state.
...
*"is the theme matching with the theme?"* over a beat that was already brand-correct navy. The
real fault was flat gradient voids, and the fix set the standing rule: **every graphic sits over
the presenter's footage**, dimmed or blurred, never on bare colour. Also: the tally super over the
face was removed entirely (the hook is now a clean face plus caption), chest overlays moved to
y908 against a per-frame chin measurement, and the keyboard bed came out. *"is this IBM flex
font?"* was a real defect: the chunk emitter linked `src="assets/..."` and never matched
...
*"just show the a-roll with the tools that you are using. Not this b-roll."* The reviewer also
asked for lines and more space on the hierarchy beat and for the product tile lifted away from the
chin. One note's markup box spanned the presenter's own chin: **the overlap had already been
priced in**, so honouring the drawn geometry beat re-deriving a safer placement.
...
### Round 4 (v7): the split doubled the face, and the music died at 35s
*"frame is repeating on the top"* was the split's top pane tweening in over 7 frames while the
full-bleed A-roll still showed the face above it. **State changes at a cut, never across one.**
...
a plausible offender while the reviewer was naming a category: any added sound at all.** The film
is now voice plus music only. See PROFILE.md rule 8.
...
with six HTML tool-UI mocks swiping above the presenter; the context beat gained drawn connector
lines and three flowing packets; **block E went from 7 cuts to 3 held shots**; and the endcard
became the client's own outro clip, with its own audio in the bed. *"The split screen should be
there, bro. What the fuck?"* is the note that produced the split's final shape.
...
a time. This round also produced the b-roll constraint rule: **the client's own b-roll cannot play
the problem half of the script**, and the blurred variant is the compromise.
...
rounded-rect vocabulary. The reviewer saw *"no difference"*. Also settled that a crossfade needs a
real underlay on both sides: fading a translucent panel over the presenter ghosts their face,
rejected twice.
...
## The client UGC reel, 9:16, 1080x1920, 41.4s (`hfad/`, 2026-08-04 to 2026-08-07)

The first production on this account. AI avatar of a woman with a cloned voice, generated b-roll,
and the client's real UI. **Local review reached v16; the client link is still on v12**, and 45 of
the project's 57 notes came from the client through the hosted share. All 57 are marked resolved.
...
- **A blur box is a composite too.** Defocusing a laptop screen with `crop`, `gblur`, `overlay`
  ...
  the bezel onto the cushion and the client called it *"the window is being stitched"*. They were
  describing the blur, not the UI under it. Fix: a feathered polygon mask of the actual glass quad,
  pulled in about 6px so the soft edge lands on the bezel.

### Round 3, v7: porting the client's own motion system
The orb went in, lifted from the orb motion lab as canvas draw code. Two other things landed:
word-level retiming fixed *"the text is not in sync"* (the build typed the wake phrase at 15.87s
and they say it at 18.83s), and the comparison scene's columns were rebuilt to actually **grow**,
...
The product window sat still for about 4s from 15.5 and the client wanted the laptop shot from
19.5 moved up to cover it. Only 5.03s of that generated take existed and 4.6s was already spent,
so a new lead-in was generated to **end on the approved take's first frame**. The two clips join
with no cut at all (colour across the join `86766b` against `877569`). The second hook line was
**dropped** rather than flashed: the new cut left it 0.76s, and **a hook line needs about 1.5s of
frame to be worth showing.** Music switched to Mixkit `gear` here, replacing the rejected AI bed.

### v3 to v6 (2026-08-05): the format pivot that produced the current card grammar
v2 was rejected too. The client sent three Cursor / trycursor Instagram ads and said *"like this
reference images on the a roll, are you clear now"*, meaning **the format itself was wrong, not
the content inside it.** The card-over-A-roll grammar reverse-engineered from those ads is in
GRAMMAR.md and is still the approved system for an avatar-led cut.
...
v1 was rejected for two structural faults: abstract graphics instead of the client's real
interface, and the presenter's face on screen 66% of the time. Rebuilt from the shot spec rather than
iterated. Face fell to about 14% full-frame plus 10.8s as a card. This round established that
**the client's own exports beat anything reconstructed**, and that captions over the presenter's cream sweater
need a scrim.

### v1 (2026-08-04): the first production on this account
The client's own icon system was the whole design: five layered elliptical stages, so a dictation
app is one ring and the client product is five, and both graphics beats became the same object in
two states. Lint-clean and still visually wrong: no display type at all, rings too thin to read at
30fps, and about 600px of dead vertical space. Frame QA at readable size is what caught it.

### Shot 01, cloned-voice audio (`client-brand/ugc/`, 2026-08-04)
The avatar's VO. Two findings are specific to this voice and worth keeping: **the presenter's real
voice is unusually concentrated at 400 to 700 Hz (50% of total energy)** and the generated read
had only 33% there, and **their measured delivery is 3.62 words/sec with clause pauses of 0.20 to
