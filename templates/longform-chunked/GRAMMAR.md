# longform-chunked: visual grammar

The current approved system is **vid62 round 3** for long-form and **vid58's short rebuild
(hf58s/)** for vertical. Reference builds: `reference-builds/longform-chunked-vid46-longform/` and
`reference-builds/longform-chunked-vid46-short/` still hold, but the sections below flag what a later video
...
| Film | Theme | Source |
|---|---|---|
| vid39 | "Plexus Indigo", then re-skinned to light editorial in round 2 | The client's own site, which is physically on screen behind their head |
| vid46 | "Signal Blue" | incogni.com's own CSS variables |

**Check what is IN the raw footage before researching an external palette.** The A-roll carries a
wall screen behind the presenter for the whole take, and a designed scene that clashes with it looks wrong
...
| Labels, mono, the coupon code | Geist Mono |
...
trade-off title. The client picked this over sans-only.
...
LED strip child using `instanceColor`. It is the one device the client named as good, twice. Do not
...
- **Solve scale, x and y per card window** from that window's own frames, not once for the take.
  The head ran 1118 to 1548px tall and its centre swayed +/-180px across one take. Thirteen of
  fourteen windows held a house 0.82; one dropped to 0.742 because the presenter leans into camera.
...
the choice between CARD and BAND for a given beat came down to a measured constraint (the contour's
sweep against the card's width), not a preference: a beat failing a residual/framing threshold was
reread by hand against both sway extremes before being demoted to the wider BAND, and CARD
...
a CARD widened to full frame on the way down and slid a full-bleed strip of the neck out past an
empty card border. That single fault was BOTH of the "glitchy/weird transition" notes on that cut.
...
**Centre the card on the MEDIAN head position, not on the midpoint of the extremes.** Extremes
centring maximises the smaller margin, which answers "will the face leave the rect" and not "does
this look centred". On vid62's short one 0.4s lean dragged a card 55px and the presenter sat off
centre for the whole beat; the note was "this is not centered aligned". If median centring then breaches
FACE_MARGIN, the beat cannot be a CARD at all and plays as a BAND.

**Solve the crop over the spans the presenter is VISIBLE in, not over the whole beat.** Five of
vid62 short's nine beats showed them for only part of their runtime, and a beat-wide median is a median over
frames nobody sees.

**"Full screen" is not automatically a 9:16 crop.** The client asked for a full-screen close on vid62's
short; a true edge-to-edge 1215px-wide 9:16 window put 43px of cheek outside the frame on both
sides and the chin at y1601, inside the UI band. It shipped instead as a full-WIDTH 1080x1468
picture from y106 down to the same y1574 floor the band uses: whole head, caption under the chin.
...
- Wipe **direction is a gaze decision**: eating a face band from the top takes the eyes out of
  frame first, which lets a conceal start before the gaze drops without ever showing a downcast
  frame.
