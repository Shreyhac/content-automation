# longform-chunked: shipped work and what each round changed
...
> "Round 4" below and `open-notes/`. They are still the ONLY open notes on any longform-chunked film.
...
| vid59 short | shared link v1, 2026-08-07 | 0 returned | out for the owner's own review, never sent to the client |
...
The client supplied three 4K screen recordings of their own Incogni dashboard on 2026-08-06, after all
...
**None of the three was re-sent to the client.** All three sit as local review versions
(`review/data/vid46` v2, `vid56` v2, `vid58` v1). The hosted vid46 link still serves the July v1,
which is the version the client's two open notes were written against.
...
- **vid58** got the highest-value swap available on any of the client's films: c6 S2 sits under the headline
...
### The supplied recordings, and what they can and cannot carry
...
**Before planning a beat around a window of "motion" in one of these captures, measure which KIND it
...
### Two things not to do to a shipped film of theirs

**Read the composition before proposing to replace something in it.** From one frame, vid46 c1's
"[a name] / Bay St / 1,240 signals" card looked like a fabricated stand-in worth swapping for the
...
close. The picture is on screen about 38 of the 56 planned seconds, no beat holds one placement
longer than 7.4s, and the states alternate band, band, card, band, card, off, band, card, full.

**Three rules decided the cut before a frame was drawn**, and each is one of the client's standing
notes: animation on top with captions in the middle and the presenter on the bottom (vid56/vid58);
lead with coverage, re-requests and the exposure scanner and let price be one beat near the end
(vid58); real graphics from the device kit plus stock, because the round-1 note on this very film
was *"No stock
...
Three of the nine notes were "weird / glitchy transition" and one was "this is not centered
aligned". They are two root causes, both now in GRAMMAR.md: a shared `off` state that let a card
widen to full frame on its way out, dragging a full-bleed strip of the neck past an empty card
...
midpoint of the extremes instead of on the median head position over the frames the presenter is
visible in.

Both audio notes were clean at signal level and editorial at script level: one join cut a sentence
...
The client's "show the a roll in full screen here" did not resolve to a 9:16 crop. See GRAMMAR.md; it
...
end") or third-person narration about the person in frame ("What they would tell a friend").
...
animations"). Cause: `stagger:.16` brought six cards in inside one second while the presenter spent 6.7s
...
## vid62 round 2, the owner rejected v1 for framing, and they were right

*"framing is off, [the presenter]'s face is not in frame, text is on [the presenter]'s face,
animations are later, a lot of weird stuff."*
...
the presenter actually sits: **180 frame-hits of text on the face across 23 elements**, a promise
rail across the chin (22% of the face), OneRep caveats across the mouth (15%), two headlines across
the neck, the CTA on the chest.
...
**Do not card the face just to clear it.** The obvious fix, card the presenter so graphics own a
half, cleared the face and emptied the other half of frame. Caught only by screenshotting the
composition before rendering. Fix instead: keep the presenter full-bleed, confine graphics to the
measured-safe column (the contour spanned x938 to x2568, so LEFT under 900 / RIGHT over 2600,
solved per chunk). **The top band is not automatically safe either, the hair sits in it.**

### validate cannot see contrast over an A-roll
It compares text to its declared CSS background; over video the real ground is the room behind the
presenter. Hid 22
...
Under the shipped transform the crown landed at y76 against a card top of y440: **the head was cut
...
windows the face MAY paint in, everywhere else forbidden, because a missing whitelist window only
...
| frames cropping the head | ~100% | **0** |
...
vid58 vetoed a full-frame close on a whole-take chin p97 and the client overruled it ("show [the
presenter] in full frame here with captions"). vid59's whole-take figure said the same thing (chin p97
...
caption room, paid in advance, the jaw at y1474 leaves y1478 to y1590 for two 56px lines, ten
...
have shipped a real defect. Measured directly, the contour sweeps 508px against a 560px card (26px
either side at the widest frame; at the left extreme the jaw sat at x252 against a card edge at
...
were on screen a second before either is spoken. `#brkLine` had the identical fault and slipped under
...
over the half-collapsed face; staggered to wait for the collapse, it left 0.3s of frame with an
...
own collapse direction, the face is gone in the first third of the move, so the panel can arrive
0.06s later without ever landing on it.
...
Every full-bleed column had been placed against the measured face contour; the real reach
(shoulders, gesturing hands) ran further, x1168 left, x2704 right, and every panel sat over the
presenter.
...
caption at y1396), solved from the presenter's own crown/chin composited onto the sway extremes and baked into
...
cut, then sat 5 to 6s); staging arrivals on the actual word onsets fixed both the stillness and
surfaced a real sync error: the "420+" figure had been on screen 4.5s before the number is spoken.
...
**One bake, three states, and only one may change the head size:**
...
a read; a blink is 2 to 4 samples). That turned a safety flag into an editorial decision: the
picture moves to CARD for that span (the presenter stays visible, smaller, while graphics carry the
price argument) and returns to full BAND once they are square to camera again.
...
baked onto `#faceCam` before a frame ran, and frame 0 (the Instagram cover) shipped with the
...
audio"*, and it was worse than it looked: the delivered "every one" was still at 0.129 RMS on the last sample
...
floor for three consecutive 20ms windows. The note flagged two joins; measuring all six found the same
shape, lower level, on three more. **When a note names an instance, check the class.**

*"[the presenter] is reading here in this frame, need to replace it with animations... as we cant
show the a roll here as [they are] looking down"*: this was the exact span already measured and moved to CARD in
the rebuild, on the theory that a smaller size solved it. **It didn't: an unwatchable shot at a
smaller size is still an unwatchable shot.** "Graphics are not the default, showing the presenter is" has a
precondition, that showing them is worth doing, and when the take itself is unusable for a span, the
fix is to take the picture OFF, not shrink the problem.

*"show [the presenter] in full frame here with captions"*: full-frame had been ruled out film-wide on the
...
anything the presenter does, exactly 810px of 960 for the whole take except one 1.4s punch-in to 1.152x. **A
...
reach,"* it dropped a figure and mangled another, which nearly got the most accurate line in the
take logged as its worst error. `medium` got it right. **Numbers spoken at speed are exactly what a small model
...
Related: the VO credits Incogni with "human attention on more complex cases" right after saying it
...
**Captions transcribe the presenter; graphics assert on the film's behalf.** Three stale Optery figures in
the script all understate the competitor. The graphics carry the corrected tier/price/gate and
stamp no numeral; the captions say what was said, stale figure included, a caption track is what a
deaf viewer has instead of the audio, and suppressing the presenter's own words there is a worse failure than
...
full-bleed across the face for exactly one frame because `L()` on a chunk's OWN start returns
...
Fourth production for this client, third long-form. 3840x2160 at **23.976** (first non-30fps build in the
...
silence is allowed, roughly one cue per 4 to 5s. And when the client supplies a pack, it has to carry the
majority of placements or they will not believe it was used.
...
and reading the resulting windows found visible note-reading at five timestamps inside "safe"
...
The coupon was placed on the claim that the body never comes below a certain x, a number from the
...
speakers are bolted down, so the only thing that varies is the presenter). Round 1's box had 2.9% of its
pixels in motion; the replacement has 0.0%. The fix was not a narrower card in the same place, it
was a card above the zone the hands work in.

### A supplied SFX pack may contain no usable sound at all
The client asked for the "saas sfx" pack by name. Measured (band energy plus envelope shape), not one of
...
on a timeline as supplied, that pack IS the drone the complaint named. `curate_sfx.py` cuts
...
top. The owner wrote "Has captions missing" and "Captions are missing," and was exactly right.
...
**A client note can name its own reference.** "place [the presenter] here, in card like we did on the
...
## vid56 short round 3/4, stop animating over the presenter, start showing them
...
Round 4's note, on the closing beat: *"No need to show any animation here when [the presenter] says
real is what are you going and whatever it is, just the A-roll should be shown. Any animation, error, just
A-roll with captions should be there. At the last line ... CTA."* **The client then said this explicitly
...
the last line, not for the whole beat, faceTo("card") lands the presenter in the card exactly as the caption
moves to the middle band and the offer card composes above them. They are on screen for the entire
close; the ask is layered onto them rather than replacing them.
...
them square to camera throughout. **Those were blinks.** The window's padding turned two blink
...
**The general rule, stated the way the client said it: graphics are not the default, showing the
presenter is.** Reach for an animated overlay when it's carrying information that cannot come from
them speaking (a number, a comparison, a mechanism); do not reach for it to fill a beat, and never
let it replace them at the moment they are making the direct ask.
...
Every on-screen claim verified against a primary source. The sponsor coupon code (`CODEWORD`) is
resolved from the affiliate URL `incogni.com/<partner>`, never from whisper, which renders a proper
noun like that as a phonetic neighbour.
...
- **The client reversed the duplicate-take decision.** Previously "keep both takes", now cut the repeated
...
- Grounding scrims measured, not assumed: bare type over the back wall measured 1.21:1.
...
**Rounds 1 to 3 were the owner. These two are the client themselves**, left on the round-3 final through
...
external drive the next day. **Anyone picking up longform-chunked work opens with these.**

Frames are in `open-notes/`. Link: the hosted review link for `vid46`.
...
- The 245 beat splits into a far-left figure and a far-right object with a dead gutter between
  them, and the dome runs to within about 190px of the right edge with no counterweight. The client
  circled the object, not the number.

**The nuance that matters: the boxed dome is the same `field.js` device the client praised** ("the
globe animations are nice"). A device being right does not make every instance of it right. What
they are reacting to here is placement and balance, not the object. **Do not redesign `field.js` on the
strength of this note.**

Neither note has been diagnosed with the client, so treat the reading above as a diagnosis and not
as their words. Their words are in the table.
...
1. **Above the chin is the face.** Graphics placed "on the chest" were on the nose and lower lip.
...
The client asked for `out/vid39-final.mp4` by name: "the animation and keyframing like this one, the face
...
- **Measure the named reference, do not describe it.** vid39's card is x216 to x864, y1020 to
  ...
  not a placement.** That single fact is what the client was reacting to.
...
- Colours, type and the three.js field untouched. The client confirmed those were right.
...
- Round 1 wrote "measured, never estimated" and then **estimated**. Two of three face numbers were
  ...
  "[the presenter] is not centre-aligned in the box" meant. **Get the crown from person segmentation.**
...
First reel for the client. Seven review rounds. This is where most of the client's standing rules
came from.
...
`out/vid39-final.mp4` is the file the client now points at by name when they want this grammar.
