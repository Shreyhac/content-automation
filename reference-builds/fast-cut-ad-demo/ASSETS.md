# fast-cut-ad-demo assets manifest

The newest client build, and the only build in this repo authored as **ONE file and chunked after the
fact**. The chunking scripts are the artefact worth reading here.

Shipped result: `reference-cuts/fast-cut-ad-demo-six-tools.mp4` (720p proxy).
Delivered original `out/ad2-final.mp4`: **1133 frames, 2160x3840 @ 30fps, 85.01 Mbps, 37.766667s,
...
| `Season` | `SeasonMixUprightsVF.woff2` | one element, `#greet`, the app greeting inside the rebuilt client app window |

**`SeasonMixUprightsVF.woff2` is a paid client licence and is deliberately NOT in this repo.** The
copy in the working project is the webfont served by the client's site, which is fine as reference and is not
...
- A-roll `/Volumes/EXTERNAL/client new /a roll.mp4`, 34.03s, 2160x3840 HEVC, 30fps, 32.9 Mbps. The
  client's own manual cut, untouched as instructed. The client's three jump cuts sit at 5.47s, 10.57s and
  27.40s. `3CB79055-....mp4` (101.5s) is the raw take it was cut from.
- B-roll `/Volumes/EXTERNAL/client new /broll/b1-b6.mp4`, all 2160x3840 HEVC 30fps at 25 to 32 Mbps,
  which are `IMG_9486` to `IMG_9491` re-exported through CapCut.

**Every B-roll clip is the same desk with the client's site already on the monitor, so no B-roll can play the
...
flagged the contradiction ("how can this screen have the product's b-roll?"). Surface the constraint in
...
| `assets/ui/client-app-icon.png` | 1 | the real client app icon, composited onto the rebuilt window |
| `assets/brand/` | `client-primary-white.svg`, `client-symbol-white.svg` | the client's own brand book vectors |
...
("we haven't revealed the product yet"), and a "?" tile also reads as product-shaped, so graph beats before
...
Every value is the client's own, from their Lightweight Guidelines V1.0, not invented.
...
product, not from the brand book. **The client's website is a blue-only reduction of the brand**: it
...
`hfad2/ref/competitor-viktor-reel.mp4` in the working repo. **"Smooth and subtle but so elegant"
