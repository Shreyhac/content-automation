"""Map reference shot boundaries onto HIS delivery, word by word.

The two scripts are the same words (he recorded the reference transcript
...
spoken, then place the cut on THAT word's onset in his take.
...
REF = words("/private/tmp/claude-501/-Users-shreyansharora-Desktop-shreyansh-claude/05f2d98e-b2c9-46bb-95c2-65c0d13d8323/scratchpad/ref3/refaudio.json")
HIS = words("hf67/transcript.json")
print(f"ref {len(REF)} words ({REF[-1][2]:.2f}s)   his {len(HIS)} words ({HIS[-1][2]:.2f}s)")

sm = difflib.SequenceMatcher(None, [w[0] for w in REF], [w[0] for w in HIS], autojunk=False)
pairs = []                      # (ref_onset, his_onset, word)
for a, b, n in sm.get_matching_blocks():
    for k in range(n):
        pairs.append((REF[a+k][1], HIS[b+k][1], REF[a+k][3]))
pairs.sort()
print(f"{len(pairs)} anchored words of {len(REF)}")

def remap(rt):
    """reference time -> his time, piecewise-linear between anchored words"""
    if rt <= pairs[0][0]:
...
def snap(t):
    """land the cut on the nearest word onset in HIS take (within 0.18s)"""
    best, bd = t, 9e9
    for w in HIS:
        d = abs(w[1] - t)
        if d < bd:
...
print(f"\n{'ref':>7} {'->his':>7} {'snapped':>8} {'d':>6}  word at that moment")
rows = []
for rc in REF_CUTS:
    ht = remap(rc)
    st, d = snap(ht)
    near = min(HIS, key=lambda w: abs(w[1] - st))
