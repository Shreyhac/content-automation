# vid46 — design system: "Signal Blue" (Incogni × Nader, sponsored long-form 16:9)
...
Third production for Nadernejad, second long-form. Inherits vid44's chunked architecture and
...
| `--ink2` / `--ink3` | `#131317` / `#1B1B20` | — | raised panel / sunk well on dark |
| `--line` / `--line2` | `#2A2A31` / `#3A3A43` | — | hairlines on dark |
...
| `--greenText` | `#4ADE80` | — | green type on dark (WCAG-safe) |
...
2. **R2 — ONE CONSTANT `tx` PER WINDOW. The tracking curve is gone.** Round 1 gave each window
   ...
   right at the median. The owner's read of the result was *"why is Nader's frame always moving
...
- **The coupon lockup** — `NADER` in Geist Mono on a blue chip, 60% OFF, `incogni.com/nader`,
