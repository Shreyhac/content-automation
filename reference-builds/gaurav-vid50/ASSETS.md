...
**gaurav's current grammar.** The paper split band as approved, three review rounds all spent on
the hook, and a build where the hook was already in the footage rather than drawn on top of it.

Shipped result: `reference-cuts/gaurav-vid50-current.mp4`. Measured from the delivered file, not
from the log: **835 frames, 1080x1920, 30fps, 27.866s, 25.61 Mbps**. Sixty timed elements.

Scaffolded on hf47 v2's approved paper split, so read `gaurav-vid47/` alongside this one: v47 is
where the split band was solved, v50 is where it settled. `creators/gaurav/HISTORY.md` walks the
three rounds.
...
This build is the clearest example of gaurav's own supplied pack in use. The `s1` through `s17`
files are his `saas` pack, which ships in this repo at `library/sfx/saas/` and which **he expects
to hear**. The `imp`, `rise`, `wsh`, `shine` files are the house pack at `library/sfx/house/`.
...
```bash
cp -R reference-builds/gaurav-vid50 work/vidNN
...
`/Volumes/Shreyansh/Reel-Factory-Archive/archive-2026-08-13/`, `projects/hf50/` and
