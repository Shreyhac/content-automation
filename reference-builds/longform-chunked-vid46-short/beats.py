...
--------------------------------------
v1 changed the face's placement 5 times, by CUTS, and the presenter's head changed size every time
the presenter appeared. v2 answered that with one fixed band. v3 keeps the discipline but adopts
...
(clip-path + scale together, 0.34s), never as a cut.
  ...
  OFF    collapsed to nothing (graphics beats: the gaze map forbids the presenter's face)
...
BEATS = [
 dict(n="b1", sent=0, f=99, a=0.00, act="A1", face=True, mode="SPLIT",
      ...
      art="the record: NAME / ADDRESS / PHONE bars slide off on their word onsets, "
          "in the TALK graphics zone BELOW the presenter's face"),
 ...
 dict(n="b9", sent=38, f=100, a=216.02, act="A5", face=True, mode="HERO",
      ...
      art="the face returns for the sign-off; the coupon holds as a chip below it. "
          "The band wipes out from 218.72, just before the presenter's gaze drops"),
...
# inside one. (This is why b1 runs to 3.30 rather than to 2.92: dropping the presenter's
...
FACE_CLIPS = [
    ...
    dict(clip="a5", src_a=215.72, src_b=219.0533, beat="b9",
         note="starts 0.30s BEFORE b9's audio (safe from 215.3), so the presenter's face is "
              "already settled when the sign-off begins. It ends at 219.053, "
              "one frame past the end of the conceal wipe: the presenter's gaze drops from "
              "218.9, and the wipe starts at 218.72 and eats the band TOP-DOWN, so "
              "the presenter's eyes are already out of frame before the first downcast frame."),
]
