...
# The presenter says CODEWORD; whisper hears CODEWURD. It is a coupon code: it must never
# reach the screen wrong. CODEWORD and the CODEWURD/CODEWORT/CODEWARD variants below are
# placeholders: replace with the sponsor's real code and the variants whisper actually
# produces for it.
for w in merged:
    if w["word"].upper().strip(".,") in ("CODEWURD", "CODEWORT", "CODEWARD"):
        w["word"] = "CODEWORD"
...
for b in beats:
    ...
    want = want.replace("CODEWURD", "CODEWORD").replace("Report, confirmed", "Report confirmed")
