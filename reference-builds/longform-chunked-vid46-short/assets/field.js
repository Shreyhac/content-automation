/* ============================================================================
   vid46-short — THE BROKER FIELD, VERTICAL (1080x1920).

   Ported from hf46/assets/field.js — the one device the owner singled out as
   "nice, I like those". The code is unchanged; only the layout DIMENSIONS are
   re-derived, because PPU is H/VIS_H and H went 2160 -> 1920, so every px
   constant in the original would come out 12.5% larger in a frame a third as
   wide. The wall is now 25x30 (tall) instead of 30x25 (wide), the twin domes sit
   at +-250px instead of +-820px so both fit inside 1080, and the plate is 52x34
   instead of 84x54 to hold the same angular size.

   Beat map for the short:
     b1  scatter   a loose cloud behind the hook, barely lit
     b4  wall      750 registered broker groups build into a slab
     b5  dome420   420 wrap a sphere; the Deloitte ring sweeps it green
     b6  twin      420 (dense) against 200 (sparse) — same radius, so the
                   comparison is DENSITY, not two different-sized props
     b11 converge  the field collapses into the coupon, all green
...
(function (global) {
  ...
  global.createField = function (opts) {
    ...
    var DOME_R = 372;                          /* px — dome420's radius           */
    ...
    L.scatter = (function () {
      var a = blank();
      for (var i = 0; i < COUNT; i++) {
        a.p[i * 3]     = jitter[i][0] * u(1500);
        a.p[i * 3 + 1] = jitter[i][1] * u(2900);
        a.p[i * 3 + 2] = jitter[i][2] * u(1600) - u(560);
        eul(a, i, jitter[i][1] * 3.1, jitter[i][0] * 3.1, jitter[i][2] * 3.1);
      }
      return a;
    })();

    /* WALL — 25 x 30 = 750 exactly.
       The pitch must EXCEED the plate, or the plates merge into continuous bars: at
       830px across 25 columns the pitch is 33px against a 52px plate, so every row
       fused and the field rendered as a barcode of horizontal stripes (caught by
       reading frames, not by any gate). 1650 x 1480 gives a 66 x 49 pitch against
       52 x 34, i.e. a real gap in both axes. The wall is now wider than the 1080
       frame, which is correct for this beat: 750 registered groups should run past
       both edges. */
    L.wall = (function () {
      var a = blank(), COLS = 25, ROWS = 30;
      var pxW = u(1650), pxH = u(1480);
      var gx = pxW / COLS, gy = pxH / ROWS;
      for (var i = 0; i < COUNT; i++) {
        var c = i % COLS, r = (i / COLS) | 0;
        a.p[i * 3]     = (c - (COLS - 1) / 2) * gx;
        a.p[i * 3 + 1] = ((ROWS - 1) / 2 - r) * gy;
        a.p[i * 3 + 2] = jitter[i][2] * u(30);
    ...
    var TWIN_R = 196, TWIN_LX = u(-250), TWIN_RX = u(250);
    ...
    L.converge = (function () {
      ...
      for (var i = 0; i < COUNT; i++) {
        ...
        a.p[i * 3]     = Math.cos(t) * u(26) * jitter[i][0];
        a.p[i * 3 + 1] = Math.sin(t) * u(26) * jitter[i][1];
        a.p[i * 3 + 2] = jitter[i][2] * u(26);
        a.s[i] = 0.001;
        eul(a, i, 0, 0, t);
      }
      return a;
    ...
    function paint() {
      ...
      if (ring.visible) {
        ...
        ring.scale.setScalar(Math.max(u(DOME_R) * rr, u(92)) + u(92));
      }
