...
reactions. The fast-cut-ad demo film went v1 to v20 in about twenty hours. vid63 took four rounds on the theme alone.
...
1 of every film in here, and round 1 in here is usually rejected. **This document is the owner's
review, run on yourself, before they see it.** It substitutes for round 1 and round 2. It does not
substitute for showing the owner the cut, and the last section says why.
...
- **Every pass has a stop condition.** If a pass finds something, you fix it and re-run that pass.
  You do not carry a known defect forward to see whether the owner notices.

Budget: about 40 minutes for a 35 second reel the first time, most of it in pass 2. That is one
review round of the owner's, and one of those costs a full re-render plus a day of latency.
...
- **Not in the browser and not scrubbed.** The owner watches the exported file in a player. That is where
...
whose window falls between two ticks, and a spliced-out grid scene on the fast-cut-ad demo film
played as bare footage
...
1. **Is anything drawn on the presenter's face?** See class 4 below. This is the single most recurrent note in
   the archive.
...
as build notes. "One beat of price, at the end" sat over the CTA for four seconds. "Their own
Incogni account" and "What they would tell a friend" were third person about the person whose face
was in the same frame.
...
- **Every join.** "The audio cuts weird here" is usually a script fault, not an encode fault.
  ...
  vid62's short's audio notes were clean at signal level: one join cut the sentence mid-list, the
...
- **Verify a suspected defect against the master.** "The A-roll repeatedly says 'pit lips, pit
  lips'" was not a repeat: the presenter had delivered the phrase as separate stabs and the cut had removed
...
- **Count your added sounds and ask whether they should exist at all.** "Remove the typing sfx"
  survived three correctly-diagnosed, correctly-measured fixes across three rounds because the
  client meant every added sound. The correct move at round 2 was one question, not three rounds of
...
## Pass 5: the rejection taxonomy, in the reviewer's words
...
| 11 | "the animation looks childish", "this looks very weird" | A retired visual family came back | `templates/<name>/GRAMMAR.md` has the retired list per account. Check it before you draw | Remove the family, do not restyle it |
| 12 | "make it stagnant", "no need to zoom the frame" | Camera grammar is per-creator and is not a house default | Grep your file for scale tweens on the stage and on any A-roll wrapper. One creator's whole zoom grammar was ported into another's film without anyone checking whose it was | Delete the camera moves. Element-level entrances are still fine |
...
The fast-cut-ad client asked for a sub-1.5s mean shot in round 1 and by rounds 6 and 9 was asking
for held b-roll and
"smooth and subtle", measured against a reference ad with zero detectable cuts.

- Read `templates/<name>/HISTORY.md` for what was reversed and when.
...
- The `.docx` behind any "comment X and I'll send you Y" line, at FIRST delivery and not when the
  owner asks. `tools/deliver/make_cta_doc.py`.
...
This protocol substitutes for round 1 and round 2. It does not substitute for showing the owner the cut.
...
What this protocol buys you is that the owner's round 1 is about the film rather than about defects you
...
the owner watched the render. The round-2 mock sheet that replaced it drew zero notes on the theme. Cost
