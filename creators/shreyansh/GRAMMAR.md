...
| vid57 | his back wall is already ivory, so ink type sits directly on it and nothing reads as pasted over him. |
...
the third dark ground this account has rejected.

- Ivory paper, ink, **one** alert red. Every device is physical: marker ring, margin handwriting
  (Gaegu), rubber stamp.
- **The fault he names as "vibe coded" is not darkness and not any one colour: it is anything that
...
- **Solve the card scale, do not type it.** A card of width W at scale s shows W/s source columns.
  vid64's s=0.70 shows 800, which puts his head centre on the card centre while the video still
...
- **Solve from the worst case, and from the contour, not the bbox.** He leans in and drifts right by
  the CTA (vid65: face centre x555 at t=0.5, x620 at t=23.0). vid66's split was solved from the face
  contour: worst face-left x598, worst face-right x1048.
...
- **Size the caption band against its worst-case line count.** vid64 shipped 66px at line-height
  ...
  every two-line caption put its second line on his face, twice in one film.
