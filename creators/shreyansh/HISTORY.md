...
segmentation per horizontal band found **0% free background y1000-1600**: he held an action
...
- **He named a device already shipped: measure it, don't remember it.** Thresholding the
  bright region of a `vid42-final.mp4` frame gave the card exactly: **652x900, centred x214,
  top y900**. Fill-width scale = 652/1080 = **0.6037**, rendering his 1003px head at 605px
...
his face read as a noisy low-quality video, the opposite of intent.
...
the take he pointed at; a sibling file in the same export folder was a *different* script
...
breakdown.md` already had the number that proves him right (1.3 head-widths against a 4.0
...
**The note's markup box caught a bug the note wasn't about.** His rectangle around one element
...
A/roll cut from a 47s raw take: no "file he sent" to byte-match. Matched the cut 4K master
...
contrast/saturation was applied in the transcode because the measured stats invited it; he
explicitly vetoed touching it. **The pipeline documents a grade step and the numbers will always
look improvable; do it anyway and it reads as touching his footage. Measure the cast, REPORT
it, don't apply it**, and verify the shipped A-roll against the master (p95 R/G/B within a
couple of units). Audio cleanup is a separate axis he hasn't objected to: say so explicitly so
he can veto it too.
...
that … we decided it to be on that size itself."* He was right: the raw 1080x1920 render
...
66→9, which was measuring the ivory cards, not his footage.

### "Don't touch the audio" reaches `deliver.py`

`deliver.py` loudnorms to -14 LUFS first: audio processing he vetoed for this video. Measured
instead: render mix -21.83 LUFS vs his source -22.25 (SFX bed adds 0.4dB, nothing else). Added
...
### His room is the design system

**His back wall is already ivory**: the same paper the approved hf55 theme is built on. Ink
type sits directly on his wall on face beats and nothing reads as pasted over him. Subject-
material rule pointed at the *footage* rather than the subject: **before inventing a ground,
check what colour his room already is.** Band law: A y170-470 (wall) · B y510-1300 (face/
...
### Face structure: he wants the CARD, not full-bleed
...
### vid57 round 2: the camera is a per-creator setting, and "animation" means a CHARACTER

**1. "The entire frame just zooms in and zooms out a lot of times. Make it stagnant."**
`feedback_premium_motion_grammar`'s wrap-every-scene-in-a-rig zoom entrance is a **thepmfguy**
finding, applied here without ever checking if he wanted it. Removed globally: `rigIn` scale
entrances, all `punch()` calls, `par()` parallax, the slow push on his A-roll. Element-level
entrances (lift/drop/fade/slam/pop/fly) stayed: those are things arriving, not the camera
moving. **Camera grammar is per-creator, not a house default: a rule filed under one creator is
evidence, not law.**
...
(a terminal); he meant a **character**. Third time this pattern appeared (vid53's Spider-Man was
the first): **when he asks for an animation of a thing, he means a drawn character that acts,
not an interface that represents the thing.** Shipped: a CSS bot (head, eyes, mouth, antenna
...
quiet; the cycle really had gone from ~4 min to ~20, and he stopped to ask why it was taking so
long. **When a step is about to cost noticeably more than the previous one, say the new number
...
Chin measured at 11 points over 29s (worst y1080), plate placed at y1190. It cut across his neck
in the CTA: he leans in over the last three seconds, chin there is y1145. Re-measuring the CTA
window alone gave chin y1145/collar y1280; plate moved to y1300. **A sparse sample of a talking
head measures the pose he holds, not the pose he moves into.** Measure the window each placement
actually plays over, and measure the beat where he's most likely to change (the close).
Corollary: a skin-mask heuristic returned "chin y424-492"; it had locked onto his glasses. **A
...
A wash over his shirt measured 1.08:1 (eyebrow) and 1.56:1 (label). Deepening the wash means
redoing it every time he moves. **A real paper card gives every glyph a known background**: the
...
the same film. His note named only one beat; the fault was two characters doing one job. **The
...
rows: a head, always. Card had to get NARROWER (560x700 → 1349 rows) to show more of him.
...
s=1 a 1080-wide band shows exactly 700 source rows while his crown-to-chin is 720. Band had to
...
His note: *"need better and relevant animations here."* Fix: show the actual verbose prompt, the
...
on a full-bleed beat his video already covers y1600-1920 and nobody reads it as a violation; the
reserved-zone rule is about TEXT and his CHIN, not pixels. Running to y1920 buys **1140 rows at
...
master's bitrate, which is the number he judges against.
...
generalises: **the scene's structure arrives on the cut, its content arrives on his words.**
Scenes open on empty/skeleton states that fill as he speaks, never on a hole. **A placeholder
...
- **Audio**: his voice is bit-faithful (correlation 0.986 once aligned) but sits **21.31ms**
...
**stop opening on his full face**: split-screen or card, only at important beats (this last
...
lifted. **Report the experiment's result, don't quietly drop it**: he asked to try something,
...
column (x440-1080, x=250): chosen because s=1 shows 640 source columns and his head spans
...
show him at 1:1 without a punch-in**: a full-width band of height H shows H source rows and his
chin lands wherever the band puts it (usually deep in the reserved zone); a full-height column
...
preview whether to skin the film in Strix's green-on-black TUI; he picked it, then watched it:
...
**stop** opening on his face; he meant **keep** the 1-1.5s open and split *after* it. **When a
brief describes current behaviour before asking for a change, the described behaviour is the
baseline being kept unless he says otherwise.** The confidence gate got spent on the theme (the
...
alert red: marker ring, margin handwriting (Gaegu), rubber stamp, all physical. **The fault he
names as "vibe coded" is not darkness or any one colour: it's anything that looks like it could
...
- **A full-width split band has to reach the bottom edge.** `inset(820px 0 340px 0)` stopped at
  y1580 to "respect" the reserved zone, leaving 340px of bare paper under him. **The reserved
  zone protects his chin, not his chest.** Extended to `inset(820px 0 0 0)`: same rule as vid60
...
### vid63 round 2, one note, and his framing on it: "if the hook is not fine, the audience drops"

Asked for the hook to be literal: a hooded cartoon hacker at his laptop, firing at it. Built flat
ink shapes, no gradient, no glow (the fault he keeps naming is anything that looks generated):
...
- **A shot needs a muzzle.** Bolts appeared already in flight. A one-frame radial flash at the
  origin (~0.18s) is the cheapest fix and the one that makes "he fired at it" read.
...
also described in his own words ("switch to split screen for animations, this animation is also
very shitty") and got built as a *different* animation (a red pen marking up code) instead of
being asked what he actually wanted. **When a note says an animation is wrong but doesn't specify
...
He reported the file "just stuck here, not playing." **Without testing anything**, reasoned from
...
```
A-roll master (his own HEVC file)    HUNG >25s
vid63-final.mp4 (H.264 delivery)     ok in 1s
vid63-final-hevc.mp4 (HEVC "fix")    ok in 1s
```

Both of *my* files opened instantly; his own untouched original was the one that hung: ruling
...
actual files takes seconds and would have shown "his file hangs too, mine don't" on the FIRST
report: immediately ruling out the encode and pointing at his file or the OS preview cache.
