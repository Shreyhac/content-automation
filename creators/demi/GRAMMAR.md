...
|---|---|
| Demi UGC reel (`hfdemi/`, v16) | **Product card over A-roll**, the Cursor-ad grammar. The A-roll is soft behind the card except at the hook, the turn and the ask |
| demi2 "six tools" (`hfdemi2/`, v20) | **Full-bleed talking head as the spine**, fast static crop-cuts, real b-roll, a two-pane split for the tool block, organic drawn overlays. **This is the current approved system**, delivered 2026-08-14 and awaiting the owner's verdict |

The delivered demi2 cut is in `reference-cuts/demi-demi2-six-tools.mp4`. Watch it before
rebuilding anything below.

What died on the way: abstract brand-geometry graphics on a bare Demi ground (v1, rejected), a
product-UI rebuild that was still not the reference's format (v2, rejected), and on demi2 a
persistent product pill plus blobs and squiggles (v13 to v17, all removed).

---

## Palette

Demi's own brand book, page 18. Not invented, and **not the website's** (see PROFILE.md rule 2).
...
The one theme note on demi2 was *"is the theme matching with demi theme?"* over a beat that was
already brand-correct navy. The fault was **flat gradient voids**. Every graphic in the approved
cut sits over his own footage: the tool grid plays over him dimmed, the app block over him
blurred. A Demi-blue panel with nothing behind it is the defect this rule exists to prevent.
...
| role | family | notes |
|---|---|---|
| display | Season Mix VF, wght 300 to 400 | Statements only. **Paid licence, not owned.** demi2 shipped without it entirely |
...
family. Both films are Plex throughout, on his instruction.
...
**The exception, measured in round 9.** He supplied a competitor ad (a Viktor AI-employee spot,
79s) and called it *"smooth and subtle but so elegant."* Scene detection found **no cuts at all**
in it, even at a 5% threshold: one continuous take, all visual interest from soft persistent
...
## Face and layout, demi2 (the current system)
...
**Three states, and the film opens without the product.**
    ...
    OPENER   a blurred hands crop, boxblur 12:2 applied at cut time in ffmpeg
             Demi is never on screen before "lives on my Mac"
    FULL     full-bleed A-roll, the spine of the film, with static crop-cuts for density
    SPLIT    a real two-pane split: cream pane on top, him cropped into the bottom pane
...
- The split was **flipped four times** before it settled. The final shape is the owner's own:
  animations in the top pane, him in the bottom, captions at the seam between the two.
- **The split arrives fully formed AT the cut.** Tweening the top pane in over 7 frames left his
  face still showing above the pane's face and he read it as *"frame is repeating"*. A state
  change happens at a cut, never across one.
...
|---|---|---|
| his chin, measured per frame from the master | about y810 to y900 | |
| chest-band chips and supers | y908 | first round's note was *"place these at the chest of the man, not on top of face"* |
| the Demi app tile in the hierarchy beat | y920 | clear of his chin |
| the six tool icons under it | y1210 | |
| captions | y1245 to y1470 | above the Meta bottom UI and clear of where his hands come up |
...
- The hierarchy beat draws **trunk, rail and six drops** as dashed connector lines from the Demi
...
Reverse-engineered from the three Cursor / trycursor Instagram ads he sent as the literal target:
...
- The A-roll runs **sharp and full-frame at the hook, the turn and the ask**, and is **softened
  ...
  blurred-behind and a small inset card at once**; he explicitly rejected that combination.
- **The UI floats as a card**, about 860px wide, with a real drop shadow. Not full-bleed, and not
  a screenshot pasted into a box.
- **A label chip straddles the card's top edge** and renames per beat (`Demi · macOS`, then
  `Slack`, then `Pending approval`). This is what reads as "the ad is telling you what you are
  looking at".
...
- **One spoken phrase, boxed, at the bottom.** Not a caption bar: a solid dark rounded chip that
  pops per word.
- Captions over her A-roll need a scrim: her cream sweater occupies exactly the y1180 to y1320
...
**The card over her face is why the format works**: with no lip sync visible behind a card, the
...
`demi/orb-lab/` is the client's own `demi-orb-motion-lab`. It is **the product's signature in the
cut** and it appears at three points in video 1: the fn-hold b-roll, the "Hey Demi" wake moment
(monochrome blooming into the Demi gradient), and hovering over the Slack UI while a thread loads.
...
It was pulled back out of beats he had not asked for it in, twice. Use it where he asked.
...
Three words take Momentum Orange across the whole of demi2 and no more: **six**, **Demi**,
**both**.
...
- No impacts, no risers, no glitch, **and as of demi2 round 5, no added sound of any kind.** Demi
  is "omnipresent, not intrusive".
...
- No drop-shadows on demi2's graphics. Glow only.
