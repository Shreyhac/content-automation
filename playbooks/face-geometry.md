...
client's note was "the presenter is not centre-aligned in the box".
...
middle band, where the presenter is. 180 frame-hits of text on the face across 23 elements, in one
film: a promise rail across the chin (22% of the face), caveats across the mouth (15%), two
headlines across the neck, the CTA on the chest. In full-bleed the A-roll paints 1:1, so the
...
right: **box for painted elements, glyph rects for bare text** (a dark card occludes the jaw
...
|---|---|---|
| longform-chunked, 16:9 4K | centre x1857, y793, crown 180, chin 1414, head 1228 | card `inset(440.1 180.2 440.3 2280.4 round 40)`, scale .82 per window |
| paper-split, vid47 | after `crop=1854:3296:36:212`: head 430px, centre x540.6 | band top y1080, video `top:380`, `inset(700px 0 0 0 round 36px)`, crown y1130 chin y1560 |
| card-reel, vid42 | crown y45 to 85, eyes y540, chin y980 to 1020, head ~940px | band top y920, video top 540, scale 1.0, `inset(380px 0 0 0 round 44px)`, chin y1500 |
| card-reel, card | head ~975px | card top **960**, hair meets card top y960, chin y1540 |
| longform-chunked, 9:16 short | chin at y1921 of 2160 (89% down, no chest) | no full-bleed exists; band 1080x1000 at y0, or card 560x736 at x260 to x820 |
...
A take where the presenter held a prop through the whole shot had **zero usable graphics zone** below y1000,
...
- **`measure_body_edge.py`**: per-pixel **temporal variance** over the window. The wall, chair,
  monitor and speakers are bolted down, so the only thing that varies is the presenter: the mask owes
  nothing to skin tone or clothing. One placement measured "safe" against a face box had **2.9%
  of its pixels in motion** from a hand; the replacement, placed against the real reach, had
  0.0%.
- **`fullbleed_guard`** (same technique, applied to graphic-zone gutters): a face contour measured
  x1409 to x2407; the real reach, shoulders and gesturing hands, was **x1168 to x2704**. Every panel
  in the film had been placed over the presenter.

The fix is never a narrower graphic in the same place; it is a graphic placed above or beside the
zone the body actually works in.
...
## Sparse sampling measures the pose the presenter HOLDS, not the pose they move into

A talking-head chin measured at 11 points over 29 seconds gave worst-case y1080, and a plate
placed off that number cut across the neck **in the CTA**, because the presenter leans toward the
lens in the last three seconds and the chin there is y1145. **Measure densely inside the window a
placement actually plays over, and measure the beat where the pose is most likely to change: the
close, where the presenter leans in to make the ask.**

A geometry heuristic can fail the same way from the tooling side: a skin-mask + width-collapse
solver built to automate exactly this measurement locked onto the presenter's glasses (which break
the skin run) and returned a confidently wrong "chin y424 to y492". **A heuristic that returns a number is not
...
130`). Hand-reading both sway extremes composited into the card showed the shot was fine: the
presenter drifts 400px right across the first 2.8s and then settles, so the card centred on where
they end up works. **The constraint a residual stands in for is whether the face stays inside the
rect this beat shows it in. Measure that directly** (the contour swept 508px against a 560px card, 26px clear
...
Under the shipped transform, the true crown landed at y76 against a card top of y440: the head was
...
Scene detection finds cuts, not gradual zooms. Face-box scale conflates a zoom with the subject
leaning in.
...
centred, and the client read it as a bug: *"why is the presenter's frame always moving left-right,
you have added some issue."*
